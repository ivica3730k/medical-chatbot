...
"""TensorFlow, Keras and TFLite versions of YOLOv5 Authored by
https://github.com/zldrobit in PR
...
class TFConv(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, w=None):
        ...
        conv = keras.layers.Conv2D(
            ...
            bias_initializer='zeros' if hasattr(
                w, 'bn') else keras.initializers.Constant(w.conv.bias.numpy()))
        self.conv = conv if s == 1 else keras.Sequential(
            [TFPad(autopad(k, p)), conv])
...
class TFDetect(keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.anchors = tf.convert_to_tensor(w.anchors.numpy(),
                                            dtype=tf.float32)
        ...
        for i in range(self.nl):
            ny, nx = self.imgsz[0] // self.stride[i], self.imgsz[
                1] // self.stride[i]
    ...
    def call(self, inputs):
        ...
        for i in range(self.nl):
            ...
            ny, nx = self.imgsz[0] // self.stride[i], self.imgsz[
                1] // self.stride[i]
            x[i] = tf.transpose(
                tf.reshape(x[i], [-1, ny * nx, self.na, self.no]),
                [0, 2, 1, 3])
...
def parse_model(d, ch, model, imgsz):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m,
        ...
        if m in [
                nn.Conv2d, Conv, Bottleneck, SPP, SPPF, DWConv, MixConv2d,
                Focus, CrossConv, BottleneckCSP, C3
        ]:
            c1, c2 = ch[f], args[0]
...
class TFModel:
    ...
    def predict(self,
        ...
        return x[
            0]  # output only first tensor [1,6300,85] = [xywh, conf, class0, class1, ...]
