...
def plot_val_study(file='',
                   dir='',
                   x=None):  # from utils.plots import *; plot_val_study()
    ...
    for f in sorted(save_dir.glob('study*.txt')):
        ...
        if plot2:
            s = [
                'P', 'R', 'mAP@.5', 'mAP@.5:.95', 't_preprocess (ms/img)',
...
def plot_evolve(evolve_csv='path/to/evolve.csv'
    ...
    for i, k in enumerate(keys[7:]):
        ...
        plt.title(f'{k} = {mu:.3g}', fontdict={'size':
                                               9})  # limit to 40 characters
        if i % 5 != 0:
            plt.yticks([])
        print(f'{k:>15}: {mu:.3g}')
