...
from utils.general import (LOGGER, NUM_THREADS, check_dataset,
...
def exif_transpose(image):
    ...
    Inplace version of https://github.com/python-
    pillow/Pillow/blob/master/src/PIL/ImageOps.py exif_transpose()
...
class LoadStreams:
    ...
    def update(self, i, cap, stream):
        ...
        n, f, read = 0, self.frames[
            i], 1  # frame number, frame array, inference every 'read' frame
    ...
    def __len__(self):
        return len(
            self.sources)  # 1E12 frames = 32 streams at 30 FPS for 30 years
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        if cache_images:
            ...
            results = ThreadPool(NUM_THREADS).imap(lambda x: load_image(*x),
    ...
    if im is None:  # not cached in ram
        ...
        if r != 1:  # if sizes are not equal
            im = cv2.resize(im, (int(w0 * r), int(h0 * r)),
                            interpolation=cv2.INTER_AREA if r < 1
                            and not self.augment else cv2.INTER_LINEAR)
        return im, (h0, w0), im.shape[:2]  # im, hw_original, hw_resized
    else:
        return self.imgs[i], self.img_hw0[i], self.img_hw[
            i]  # im, hw_original, hw_resized
...
def autosplit(path='../datasets/coco128/images',
              weights=(0.9, 0.1, 0.0),
...
def dataset_stats(path='coco128.yaml',
    ...
    To run in parent directory: export PYTHONPATH="$PWD/_yolov5"
    ...
    def round_labels(labels):
        ...
        return [[int(c), *(round(x, 4) for x in points)]
    ...
    def unzip(path):
        ...
        if str(path).endswith('.zip'):  # path is data.zip
            assert Path(
                path).is_file(), f'Error unzipping {path}, file not found'
            ZipFile(path).extractall(path=path.parent)  # unzip
            dir = path.with_suffix('')  # dataset directory == zip name
            return True, str(dir), next(
                dir.rglob('*.yaml'))  # zipped, data_dir, yaml_path
    ...
    for split in 'train', 'val', 'test':
        ...
        if hub:
            ...
            for _ in tqdm(ThreadPool(NUM_THREADS).imap(hub_ops,
                                                       dataset.img_files),
                          total=dataset.n,
                          desc='HUB Ops'):
                pass
